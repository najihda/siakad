...
function Tampilkan_Mata_Kuliah($jdwl)
{
	...
	foreach ($jdwl->result_array() as $value)
	{
		if(($temp=='') || ($value['kd_mk']!=$temp)) {
			...
			$linkpeserta = $peserta;
			if($peserta >0)
			$linkpeserta = '<a href="'.base_url().'dosen/peserta/'.$value['kd_jadwal'].'_1
			...
			if($calonpeserta >0)
			$linkcalonpeserta = '<a href="'.base_url().'dosen/peserta/'.$value['kd_jadwal'].'_0
			...
			$rows[$index] .='<td id="'.'cell_'.$value['kd_mk'].'_'.$value['kd_prodi'].'">'.$value['nama_dosen'].'</td>
				...
				<td align="center">'.$linkcalonpeserta.'</td></tr>';
			$index++;
			$temp=$value['kd_mk'];
	}
	foreach($rows as $row)
